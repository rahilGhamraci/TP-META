...
public class AlgoGenV3 {
    ...
    public int[] recherche(float probaCross, float probaMutation, int nbGen, int taillePop, int[][] dis, int numCities) {
        ...
        for (int i = 0; i < nbGen; i++) {
            ...
            // crossover 
            populationCrossOver = crossover(probaCross, populationSelectionnee, populationSelectionnee.size(), numCities);
            System.out.println("\npopulation apres application du crossover");
            afficherPop(populationCrossOver);
            // mutation
            populationMutated = mutation(probaMutation, populationCrossOver, populationCrossOver.size(), numCities);
            ...
            int indiceMax = Arrays.stream(valeursF)
                    ...
                    .indexOf(Arrays.stream(valeursF).max().getAsInt());
            if (fitness(bestSolution, dis, numCities) < valeursF[indiceMax] && valide(populationMutated.get(indiceMax), numCities)) {
    ...
    public static ArrayList<int[]> crossover(float probaCross, ArrayList<int[]> population, int taillePop, int numCities) {
    ...
    public static ArrayList<int[]> mutation(float probaMutation, ArrayList<int[]> population, int taillePop, int numCities) {
